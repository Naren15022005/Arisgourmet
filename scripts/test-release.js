...
function post(path, body, headers = {}, cb) {
  ...
  const req = http.request('http://127.0.0.1:4000' + path, opts, (res) => {
    let b = '';
    res.on('data', (c) => (b += c));
...
post('/auth/login', { email: 'dev@local', password: 'secret' }, {}, (err, code, body) => {
  ...
  post('/api/mesas/release', { codigo_qr: 'MESA-1' }, { Authorization: 'Bearer ' + token }, (err2, code2, body2) => {
    if (err2) return console.error('release err', err2);
    console.log('release', code2, body2);
  });
});
