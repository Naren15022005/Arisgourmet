...
const workerId = process.env.OUTBOX_WORKER_ID || (0, crypto_1.randomUUID)();
const counters = {
    processed: 0,
    retried: 0,
    dlq: 0,
...
async function processBatch(redisUrl = process.env.REDIS_URL || 'redis://localhost:6379') {
    ...
    const repo = data_source_1.AppDataSource.getRepository(outbox_entity_1.Outbox);
    const redis = new ioredis_1.default(redisUrl);
    try {
        const outboxExists = await data_source_1.AppDataSource.query("SELECT COUNT(*) as c FROM information_schema.TABLES WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='outbox'");
        ...
        for (const r of rows) {
            try {
                ...
                console.error('Unexpected error processing outbox id', r && r.id, inner && inner.stack ? inner.stack : inner);
            }
        }
    }
    finally {
